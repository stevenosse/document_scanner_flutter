...
class _PdfGeneratotGalleryState extends State<PdfGeneratotGallery> {
  ...
  openViewer(File currentItem) {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (_) => PhotoViewer(galleryItems: files, selectedItemIndex: index)));
  }
  ...
  String get itemsTitle {
    ...
    var singleTitle = widget.labelsConfig[ScannerLabelsConfig.PDF_GALLERY_FILLED_TITLE_SINGLE] ?? '$countHolder Page';
    var multiTitle = widget.labelsConfig[ScannerLabelsConfig.PDF_GALLERY_FILLED_TITLE_MULTIPLE] ?? '$countHolder Pages';
  ...
  Widget build(BuildContext context) {
    var appBar = AppBar(
      ...
      title: Row(
        children: [if (files.isNotEmpty) Text(itemsTitle), if (files.isEmpty) Text(widget.labelsConfig[ScannerLabelsConfig.PDF_GALLERY_EMPTY_TITLE] ?? "PDF Pages")],
      ),
    );
    return Scaffold(
      appBar: appBar,
        ...
        children: [
                                    ...
                                    children: [
                                      ...
                                      Positioned(
                                        ...
                                        child: GestureDetector(
                                          onTap: () => _removeFile(files.map((e) => e.path).toList().indexOf(image.path)),
                                          child: Container(
                                            padding: EdgeInsets.all(3),
                                            decoration: BoxDecoration(color: Colors.white, border: Border.all(width: 1, color: Colors.grey), borderRadius: BorderRadius.circular(15)),
          ...
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(boxShadow: [BoxShadow(color: Colors.black.withOpacity(.2), spreadRadius: 1, blurRadius: 10)], borderRadius: BorderRadius.circular(25)),
              child: Row(
                  ...
                  if (files.isNotEmpty)
                    Expanded(
                      child: _mainControl(
                        ...
                        onTap: onDone,
                        radius: BorderRadius.only(topLeft: Radius.circular(25), bottomLeft: Radius.circular(25)),
                      ),
                    ),
                  Expanded(
                      child: _mainControl(context,
                          ...
                          onTap: addImage,
                          radius: files.isEmpty ? BorderRadius.circular(25) : BorderRadius.only(topRight: Radius.circular(25), bottomRight: Radius.circular(25)))),
                ],
              ),
            ),
          ),
